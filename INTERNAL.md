...
- The version reported by the installed compiler must match the compiler version carried by the extension's bundled `node/moonbit-lsp`. The bundled LSP lookup prefers `moonbit-lsp.exe` on Windows and `moonbit-lsp` elsewhere, with the opposite filename as fallback.
...
- The compiler version installed by the VS Code extension must match the compiler version installed by the official CLI installer.
...
- The installed compiler version differs from the compiler version carried by the Marketplace extension's bundled LSP.
